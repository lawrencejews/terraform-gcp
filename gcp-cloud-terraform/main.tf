...
resource "google_storage_bucket" "cloud-storage-bucket-terraform" {
  name          = "terraform-gcp-bucket-terraform"
  ...
  labels = {
    "environment" = "terraform_dev_test"
...
resource "google_compute_network" "custom-vpc-network-terraform" {
  name                    = "custom-vpc-network-terraform"
